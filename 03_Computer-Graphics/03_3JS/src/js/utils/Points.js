...
export class PointsUtils {
    ...
    // Chosen names for compatibility between (2D and 3D) projects
    static BezierCurve2D(startPoint, controlPoint, endPoint, numPoints) {
        return this.quadraticBezierCurve_path(startPoint, controlPoint, endPoint, numPoints);
    }
    ...
    static reflectAcrossAxis(points, axisStart, axisEnd) {
        ...
        return points.map(point => {
            // Calculate the perpendicular line passing through the point
            ...
            // Calculate the oposite side of the point 
    ...
    static translatePoints(points, x_tr, y_tr, z_tr) {
        for (let p of points) {
            p.x += x_tr;
            p.y += y_tr;
            p.z += z_tr;
